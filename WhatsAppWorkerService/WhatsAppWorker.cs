...
/// </remarks>
public sealed class WhatsAppWorker : BackgroundService
{
        ...
        */
        ...
        rabbitConsumer.ReceivedAsync += async (_, ea) =>
        {
            try
            {
                ...
                {
                    _logger.LogWarning("[WhatsAppService] Message without properties -> reject");
                    await _channel.BasicNackAsync(ea.DeliveryTag, multiple: false, requeue: false);
                    return;
                }
        ...
        _logger.LogInformation("[WhatsAppService] Listening queue {QueueName}. Press Ctrl+C to exit.", _opt.QueueName);
        try
        {
